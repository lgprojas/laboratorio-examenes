...
<!--  Menu dependiendo rol    --> 
            ...
            {if isset($_layoutParams.menu_cl)}
                <ul class="navbar-nav mr-auto mb-2 mb-lg-0">
                   {foreach item=it from=$_layoutParams.menu_cl}
                       <li {if !empty($it.children)}class="nav-item dropdown"{else}class="nav-item"{/if}>
                           <a {if !empty($it.children)}class="nav-link dropdown-toggle" data-toggle="dropdown" href="#"{else}class="nav-link" href="{$it.enlace}"{/if}>{$it.title}{*if !empty($it.children)}<b class="caret"></b>{/if*}</a>
                       {if !empty($it.children)}
                           <ul class="dropdown-menu shadow">
            ...
            {if isset($_layoutParams.menu_ab)}
                <ul class="navbar-nav mr-auto mb-2 mb-lg-0">
                   {foreach item=it from=$_layoutParams.menu_ab}
                       <li {if !empty($it.children)}class="nav-item dropdown"{else}class="nav-item"{/if}>
                           <a {if !empty($it.children)}class="nav-link dropdown-toggle" data-toggle="dropdown" href="#"{else}class="nav-link" href="{$it.enlace}"{/if}>{$it.title}{*if !empty($it.children)}<b class="caret"></b>{/if*}</a>
                       {if !empty($it.children)}
                           <ul class="dropdown-menu shadow">
            ...
            <ul id="ulclicknoti" class="navbar-nav mr-auto mb-2 mb-lg-0">
                  <li class="dropdown notifications-menu">
                      <a data-id="ejemplo" href="#" class="navega-lucho dropdown-toggle" id="campana" data-toggle="dropdown"><i class="fa fa-bell-o"></i>{if Session::get('msj') != 0}<span class="label label-warning">{Session::get('msj')}</span>{else}{/if}</a>
                    <ul class="dropdown-menu">
                      <li class="header">
                          <div class="row">
                              <div class="row col-lg-10 text-start"><div class="col-lg-2"><i class="bi bi-file-text"></i></div>
                                  ...
                                  {if Session::get('msj') != 0}
                                      Usted tiene {Session::get('msj')} notificaciones 
                                  {else}
                      ...
                      <li>
                        ...
                        <ul id="lastnoti" class="menu">
                          {if Session::get('menu_noti')|@count gt 0}
                          {foreach item=mn from=Session::get('menu_noti')}  
                          <li class="text-li-noti">
                            <a {if $mn.Id_tmov == 1}  
                                    {if Session::get('level') == 3}
                                        href="{$_layoutParams.root}causa/index/index/{$mn.Idcli_encrypt}/{$mn.Idempresa_encrypt}"
                                    {elseif Session::get('level') == 4}
                                        href="{$_layoutParams.root}causa/index/index/{$mn.Idcli_encrypt}/{$mn.Idempresa_encrypt}"
                                    {elseif Session::get('level') == 5}
                                        href="{$_layoutParams.root}causa/indexcli/index/{$mn.Idcli_encrypt}/{$mn.Idempresa_encrypt}"
                                    {/if}
                                {elseif $mn.Id_tmov == 2}
                                    {if Session::get('level') == 3 || Session::get('level') == 4}
                                        href="{$_layoutParams.root}causa/dcausa/index/{$mn.Idcli_encrypt}/{$mn.Idempresa_encrypt}/{$mn.Idaccion_encrypt}"
                                    {elseif Session::get('level') == 5}
                                        href="{$_layoutParams.root}causa/indexcli/verListaHitosCausaCli/{$mn.Idcli_encrypt}/{$mn.Idempresa_encrypt}/{$mn.Idaccion_encrypt}"
                                    {/if}    
                                {elseif $mn.Id_tmov == 3}
                                    {if Session::get('level') == 3 || Session::get('level') == 4}
                                        href="{$_layoutParams.root}consulta/dconsulta/index/{$mn.Idcli_encrypt}/{$mn.Idempresa_encrypt}/{$mn.Idaccion_encrypt}"
                                    {elseif Session::get('level') == 5}
                                        href="{$_layoutParams.root}consulta/dconsultacli/index/{$mn.Idcli_encrypt}/{$mn.Idempresa_encrypt}/{$mn.Idaccion_encrypt}"
                                    {/if}
                                {elseif $mn.Id_tmov == 4}
                                    {if Session::get('level') == 3 || Session::get('level') == 4}
                                        href="{$_layoutParams.root}consulta/dconsulta/index/{$mn.Idcli_encrypt}/{$mn.Idempresa_encrypt}/{$mn.Idaccion_encrypt}"
                                    ...
                                    {/if}
                                {/if}>
                                <i class="bi bi-megaphone-fill"></i> {if $mn.Id_codrol == 1}[Colaborador] {$mn.Nom1_emp} {$mn.Ape1_emp} {$mn.Ape2_emp|substr:0:1}. {else}[Cliente] {$mn.Nom1_cli} {$mn.Ape1_cli} {$mn.Ape2_cli|substr:0:1}. {/if}<span style="font-weight: bold;">{$mn.Nom_tmov}</span>
                                <p>Fecha: <span style="font-weight: bold;">{$mn.Date_mov}</span> Hora: <span style="font-weight: bold;">{$mn.Time_mov} hrs.</span></p>
                            </a>
                          </li>        
                          {/foreach}                         
                          {/if}
                        ...
                        <ul class="menu">
                          <li class="text-li-noti" style="text-align: center;">
                              {if Session::get('level') == 3}
                                  <a href="{$_layoutParams.root}movimientos/index/notifSup/">Ver más movimientos..</a>
                              {elseif Session::get('level') == 4}
                                  <a href="{$_layoutParams.root}movimientos/index/notifAbo/">Ver más movimientos..</a>
                              {elseif Session::get('level') == 5}
                                  <a href="{$_layoutParams.root}movimientos/index/notifCli/">Ver más movimientos..</a>
                              {/if}
                          </li>
                        </ul>
                      </li>
                ...
                <ul class="navbar-nav ms-auto">
                   <li class="nav-item dropdown">
                      ...
                      <ul class="dropdown-menu">
                        ...
                        <li><a class="dropdown-item" href="{$_layoutParams.root}usuarios/logincli/" class="btn">Iniciar sesión cliente</a></li>
                      </ul>
                  </li>
               </ul>  
        {/if}
